...
export function setToken(data: DataInfo<Date>) {
  ...
  function setUserKey({
    ...
    useUserStoreHook().SET_COMPANY_INFO({
      companyId,
      companyCode,
      companyName
    });
  ...
  if (data.username) {
    ...
    const companyCode =
      storageLocal().getItem<DataInfo<number>>(userKey)?.companyCode;
    const companyName =
      storageLocal().getItem<DataInfo<number>>(userKey)?.companyName;
    const storeId = storageLocal().getItem<DataInfo<number>>(userKey)?.storeId;
    const employeeId =
...
export const isSuperAdmin = (): boolean => {
  const userStore = useUserStoreHook();
  return userStore.isSuper || false;
};
